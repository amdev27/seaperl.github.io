...
export function SiteHeader({ dark = true }: { dark?: boolean }) {
  ...
  return (
    <>
      <div className="utility-bar"><span>Algae biotechnology, cultivated in Ahmedabad since 2016</span><div><a href="mailto:seapearlbiotech@gmail.com">seapearlbiotech@gmail.com</a><a href="/contact">Contact us ↗</a></div></div>
      <nav className={`nav-shell ${dark ? "nav-dark" : "nav-light"}`} aria-label="Primary navigation">
        <Link className="wordmark" href="/" aria-label="Sea Pearl Biotech home">
          <img className="real-logo" src="/original/sea-pearl-3-06-768x154.png" alt="Seapearl Biotech — Algae Based Biotech Company"/>
        </Link>
        <div className="desktop-nav">
          {links.map(([label, href]) => <Link className={pathname === href ? "active" : ""} href={href} key={href}>{label}</Link>)}
        </div>
        <a className="nav-cta" href={shopHref}>Shop now <ArrowUpRight size={16} /></a>
      ...
      <div ref={drawerRef} id="mobile-navigation" role="dialog" aria-modal="true" aria-label="Site navigation" className={`mobile-drawer ${open ? "is-open" : ""}`} aria-hidden={!open} inert={!open}>
        <div className="drawer-top">
          <img className="real-logo" src="/original/sea-pearl-3-06-768x154.png" alt="Seapearl Biotech"/>
        ...
        <div className="drawer-links" onClick={()=>setOpen(false)}>
          ...
          <Link href="/contact">Contact <span>08</span></Link>
          <Link href="/applications">Applications ↗</Link><a href={shopHref}>Shop now ↗</a>
        </div>
