...
export function Certifications(){return <section className="section certification-section"><div className="cert-heading"><p className="section-label">Quality & recognition</p><a href={sitePath("/quality")}>Explore our quality approach ↗</a></div><div className="cert-logos">{certificates.map(([name,file])=><a href={sitePath("/quality")} key={name}><img src={sitePath(`/original/${file}`)} alt={name} loading="lazy"/><span>{name}</span></a>)}</div><p className="cert-note">Marks displayed on Sea Pearl Biotech’s existing website. Current certificates, licence scope and validity are available for confirmation with our team.</p></section>}
